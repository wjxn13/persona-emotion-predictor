...
| 指标 | 验证集 | 测试集 |
|------|--------|--------|
| 准确率 Accuracy (v1) | 36.15% | **34.77%** |
| 宏平均 F1 Macro F1 (v1) | 34.55% | **31.91%** |

- 随机基线（7 分类）准确率 ≈ 14.3%，v1 模型性能为基线的 **2.4 倍**
- 💡 **与 LLM 对比**：Persona-E² 论文测得主流 LLM 的 Top-1 仅约 **25%**，而本项目的 BERT 实现测试集 **34.77% 显著超过 LLM**——说明“小模型 + 性格调制”在此任务上并不弱，本项目可作为该任务的社区参考实现
- 使用 `bert-base-uncased` 重训练预计可提升至 **37%–40%**
- **v2（domain 分支 + soft label）重训后预期进一步提升**，相关权重与 `label_map.json` 待重训后从 Release 更新
